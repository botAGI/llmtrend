"""Alembic environment configuration for the AI Trend Monitor.

Supports both synchronous (offline) and asynchronous (online) migration
execution.  The database URL and model metadata are loaded from the
application's Settings and ORM models, respectively, so no configuration
is duplicated.
...
# Interpret the config file for Python logging (unless we're called
# programmatically with an existing logger setup).
...
def run_migrations_online() -> None:
    ...
    context.  The async driver (asyncpg) requires running through
    ``asyncio.run()``.
    """
